...
public class ShadowMapEntityRenderer {
	 ...
	 */
	protected void render(Map<Model, List<ModelEntity>> entities) {
		shader.loadHasAlpha(true);
		for (Model model : entities.keySet()) {
			bindModel(model);
			for (ModelEntity entity : entities.get(model)) {
	...
	private void prepareAnimatedPart(Model modelData, Matrix4f transformationMatrix) {
		GL30.glBindVertexArray(modelData.getVaoID());
